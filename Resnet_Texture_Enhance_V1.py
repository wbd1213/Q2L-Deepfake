...
class Texture_Enhance_v1(nn.Module):
    def __init__(self, num_features, num_attentions):   # num_features=64, num_attentions=1
        super().__init__()
        #self.output_features=num_features
        self.output_features = num_features * 4
        self.output_features_d = num_features
        self.conv0 = nn.Conv2d(num_features, num_features, 1, stride=2)
        ...
        self.conv_last = nn.Conv2d(num_features * 4, num_features * 4, 1, stride=2)
    ...
    def forward(self, feature_maps, attention_maps=(1,1)):
        B, N, H, W = feature_maps.shape           # torch.Size([1, 64, 64, 64])
        # print("feature_maps:", feature_maps.shape, len(feature_maps))
        if type(attention_maps) == tuple:
            # attention_size = (int(H * attention_maps[0]), int(W * attention_maps[1]))    #attention_maps=(64,64)
            attention_size = (8, 8)
        else:
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.texture_enhance = Texture_Enhance_v1(num_features=512, num_attentions=1)
    ...
    def _forward_impl(self, x):
        ...
        x = self.conv1(x)            # layer0： torch.Size([1, 64, 224, 224])

        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)

        x1 = self.layer1(x)          # layer1： torch.Size([1, 256, 112, 112])
        x2 = self.layer2(x1)         # layer2： torch.Size([1, 512, 56, 56])
        x3 = self.layer3(x2)          # layer3： torch.Size([1, 1024, 56, 56])
        x4 = self.layer4(x3)          # layer4： torch.Size([1, 2048, 14, 14])

        x_te1 = self.texture_enhance(x2)
